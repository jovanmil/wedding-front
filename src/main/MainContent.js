...
class MainContent extends Component {
    ...
    generateContent() {
        ...
        if (guests && guests.size !== 0) {
            ...
            //sort by subcategory
            guestArr.sort((a, b) => {
                if (a.get("category").get("name") === b.get("category").get("name")) {
                    return (a.get("subcategory").get("name") > b.get("subcategory").get("name")) ? 1 : -1
                }
            });

            //sort by last name
            guestArr.sort((a, b) => {
                if (a.get("subcategory").get("name") === b.get("subcategory").get("name")) {
